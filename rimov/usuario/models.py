...
from rimov.utils.gerador_hash import gerar_hash
...
class CorretorAtivoManager(UserManager):

    def get_queryset(self):
        return super().get_queryset().filter(tipo='CORRETOR', is_active=True)
...
class Usuario(AbstractBaseUser):
    ...
    corretores = CorretorAtivoManager()
    ...
    def get_absolute_url(self):
        return reverse('usuario:usuario_update', args=[str(self.pk)])

    @property
    def get_delete_url(self):
        return reverse('usuario:usuario_delete', args=[str(self.pk)])
